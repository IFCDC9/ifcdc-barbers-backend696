...
/** `barbers.business_id` may be TEXT or BIGINT; always coerce via ::text. */
export function barberBusinessIdSql(alias = "b") {
  return `CASE
    WHEN ${alias}.business_id IS NOT NULL AND btrim(${alias}.business_id::text) ~ '^[1-9][0-9]*$'
      THEN btrim(${alias}.business_id::text)::bigint
    ELSE NULL
  END`;
}
