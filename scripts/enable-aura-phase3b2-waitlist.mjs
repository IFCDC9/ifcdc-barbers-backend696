#!/usr/bin/env node
/**
 * Controlled enable of AURA_PHASE3_WAITLIST on backend696 only.
 * Keeps slot recovery, waitlist notifications, and operational insights OFF.
 *
 * Usage:
 *   RENDER_API_KEY=rnd_... node --import ./loadBackendEnv.mjs scripts/enable-aura-phase3b2-waitlist.mjs --deploy
 *   RENDER_API_KEY=rnd_... node --import ./loadBackendEnv.mjs scripts/enable-aura-phase3b2-waitlist.mjs --verify-only
 */
...
console.log(`\n=== Enable Phase 3B2 WAITLIST only ===\nService ${SERVICE_ID}\n`);

if (!VERIFY_ONLY) {
  await putEnv("AURA_PHASE3_WAITLIST", "1");
  await putEnv("AURA_PHASE3_SLOT_RECOVERY", "0");
  await putEnv("AURA_PHASE3_WAITLIST_NOTIFICATIONS", "0");
  await putEnv("AURA_PHASE3_OPERATIONAL_INSIGHTS", "0");
  console.log("Set AURA_PHASE3_WAITLIST=1");
  console.log("Forced OFF: SLOT_RECOVERY, WAITLIST_NOTIFICATIONS, OPERATIONAL_INSIGHTS");
  ...
  for (const k of [
    ...
    console.log(`Confirm ${k}=${isOn(v) ? "ON(BAD)" : "off"}`);
  }
  ...
  for (const k of [
    ...
    const v = await getEnv(k);
    console.log(`${k}=${v ?? "(unset)"} on=${isOn(v)}`);
  }
}
