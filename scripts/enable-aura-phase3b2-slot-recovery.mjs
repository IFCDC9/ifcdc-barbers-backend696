#!/usr/bin/env node
/**
 * Controlled enable of AURA_PHASE3_SLOT_RECOVERY on backend696 only.
 * Keeps waitlist notifications and operational insights OFF.
 *
 *   RENDER_API_KEY=rnd_... node --import ./loadBackendEnv.mjs scripts/enable-aura-phase3b2-slot-recovery.mjs --deploy
 */
...
console.log(`\n=== Enable Phase 3B2 SLOT RECOVERY only ===\nService ${SERVICE_ID}\n`);
...
if (!VERIFY_ONLY) {
  ...
  console.log("Set AURA_PHASE3_SLOT_RECOVERY=1");
  console.log("Forced OFF: WAITLIST_NOTIFICATIONS, OPERATIONAL_INSIGHTS");

  for (const k of ["AURA_PHASE3_WAITLIST_NOTIFICATIONS", "AURA_PHASE3_OPERATIONAL_INSIGHTS"]) {
