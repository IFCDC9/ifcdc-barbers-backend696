...
async function upsertBarberServiceStyle(dbQuery, {
  ...
  const ins = await dbQuery(
    `INSERT INTO barber_services (barber_id, name, description, category, image_url, price, duration_minutes, is_active)
     VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
     RETURNING id, barber_id, name, description, category, icon, image_url,
               price::float8 AS price, duration_minutes, is_active`,
    [barberKey, title, description || null, category || null, String(imageUrl || "") || null, p, duration, Boolean(isActive)],
  );
